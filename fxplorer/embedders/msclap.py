"""
Legacy MS-CLAP wrapper kept for reference.

This file is currently unused by the active FXplorer pipeline and is
not part of the supported default environment.
"""
...
class MSCLAPWrapper(AbstractCLAPWrapper):
    ...
    def resample(self, signal: AudioSignal, resample=True):
        """
        trying to see if resampling step in read_audio (step 1) is the issue,
        but it seems its from the very beginning w/ reading in the file
        """
    ...
    def compute_similarity(self, audio_emb, text_emb) -> torch.Tensor:
        # TODO: check whether this will give logit scaled (outside [0,1] range)
